...
_LF = os.environ.get("KML_LASTFE", "0") == "1"     # GPT막판: lastfe(quantile/gap/daily) 포함 → _lf
_LB = os.environ.get("KML_LABELFE", "0") == "1"    # 두AI label: basket충돌/대리구매/tdecay → _lb
KFOLD = int(os.environ.get("PL_KFOLD", "0"))       # >0=즉석 StratifiedKFold N (variance↓, #112식 1위레버). ⚠OOF는 5fold와 불일치→stack입력금지, 단독/단순블렌드만
_kf = f"_{KFOLD}f" if KFOLD > 0 else ""
...
SUF = ("_lf" if _LF else "") + ("_lb" if _LB else "") + ("_te" if _TE else "") + ("_ple" if _EMB else "") + ("_plb" if _BEST else ("_plc" if _CONS else ("_pl3" if ITER else "_pl2"))) + _thr + _wtag + _tk + _kf
