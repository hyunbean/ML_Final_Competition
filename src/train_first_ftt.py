"""FTTransformer_RTDL_D_Classifier (1등 피처셋) → 'first_ftt'. RealMLP/TabM과 다른 NN 구조 = 더 직교. 강한모델×1등 방향.
실행(GPU): pip install pytabkit gensim catboost → python -m src.train_first_ftt
"""
import numpy as np
...
MODEL_NAME = "first_ftt"


def main():
    from pytabkit import FTTransformer_RTDL_D_Classifier
    ...
    for f in range(C.N_FOLDS):
        ...
        m = FTTransformer_RTDL_D_Classifier(n_cv=1, random_state=C.SEED, device="cuda")
        m.fit(X[tri], y[tri])
    ...
    save_predictions(MODEL_NAME, oof, ts / C.N_FOLDS, meta=dict(cv_auc=cv, seed=C.SEED, n_folds=C.N_FOLDS,
                     feature_set="1등 FE FTTransformer_RTDL_D_Classifier", created_by="hyunbean", notes="FTTransformer_RTDL_D_Classifier on 1st-place feats"))
