...
FREQ_COLS = ["corner_nm", "brd_nm", "goodcd", "buyer_nm", "pc_nm"]


def _freq_maps(tr, te):
    """train+test 합쳐 카테고리 등장빈도 맵 (비지도, 누수없음)."""
    return {c: pd.concat([tr[c], te[c]]).astype(str).value_counts() for c in FREQ_COLS}


def _frequency_features(df, ids, maps):
    """값빈도 인코딩 — 고객이 산 카테고리들의 희귀도(Santander magic류). 희귀취향=신호."""
    gid = df[C.ID_COL]
    out = {}
    for c in FREQ_COLS:
        f = df[c].astype(str).map(maps[c]).fillna(1).astype(float)
        lf = np.log1p(f)
        out[f"freq_{c}_mean"] = lf.groupby(gid).mean()
        out[f"freq_{c}_min"] = lf.groupby(gid).min()
        out[f"freq_{c}_rare_ratio"] = (f <= 5).astype(float).groupby(gid).mean()
    return pd.DataFrame(out).reindex(ids).fillna(0.0)
...
def build_features(use_te=True, use_emb=True, use_groups=True, use_fe2=False, emb_vector_size=16,
    ...
    # 3.9) frequency encoding (값빈도/희귀도 — 새 축)
    fmaps = _freq_maps(tr, te)
    ...
    # 8) 핵심 피처 상호작용 (수치×수치 곱/비율 — 새 파생축)
    def _add_interactions(X):
        pairs = [
            ("te_corner_nm_wmean", "te_brd_nm_wmean", "mul"),
            ("fe_gender_part_score", "fe_gender_interest_score", "mul"),
            ("te_corner_nm_wmean", "net_amt_mean", "mul"),
            ("recency_days", "txn_per_month", "mul"),
            ("fe_dwell_longstay_ratio", "te_corner_nm_wmean", "mul"),
            ("freq_brd_nm_rare_ratio", "te_brd_nm_wmean", "mul"),
            ("grp_cosmetic_amt", "grp_men_amt", "mul"),       # 화장품×남성(선물/혼합신호)
            ("te_corner_nm_wmean", "active_months", "div"),
        ]
        for a, b, op in pairs:
            if a in X.columns and b in X.columns:
                X[f"ix_{a}_{op}_{b}"] = X[a] * X[b] if op == "mul" else X[a] / (X[b].abs() + 1e-5)
        return X

    # train/test 컬럼 정렬 일치시켜 합치기
    X_train = _add_interactions(pd.concat(blocks_tr, axis=1).fillna(0.0))
    X_test = _add_interactions(pd.concat(blocks_te, axis=1).fillna(0.0))
