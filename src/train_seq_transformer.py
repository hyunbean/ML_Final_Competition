"""트랜스포머 인코더 시퀀스 모델 (BERT4Rec/SASRec 스타일, from-scratch, GPU).

사전학습 BERT가 아니라 self-attention '구조'를 구매시퀀스로 직접 학습.
GRU와 다른 inductive bias = 추가 다양성. brd+part+amt 멀티채널 + 위치임베딩.
features 캐시 불필요. 5-fold 고객 단위.

실행: python -m src.folds (선행) → python -m src.train_seq_transformer
"""
import numpy as np
import pandas as pd
import torch
...
MODEL_NAME = "seq_transformer"
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
CHANNELS = ["brd_nm", "part_nm"]
AMT_BINS = 12
MAXLEN, DMODEL, HEADS, LAYERS = 60, 48, 4, 2
EMB = {"brd_nm": 24, "part_nm": 12, "amt": 6}     # 합 42 → DMODEL로 투영
EPOCHS, BATCH, PATIENCE, LR = 25, 256, 5, 1e-3
...
def _pad(df, ids):
    ...
    for cid, g in df.groupby(C.ID_COL, sort=False):
        ...
        for c in cols:
            ...
            arrs[c][i, -len(s):] = s
    return np.stack([arrs[c] for c in cols], axis=2)
...
class TxEncoder(nn.Module):
    def __init__(self, vocabs):
        ...
        self.proj = nn.Linear(sum(EMB.values()), DMODEL)
        self.pos = nn.Parameter(torch.zeros(1, MAXLEN, DMODEL))
        layer = nn.TransformerEncoderLayer(DMODEL, HEADS, DMODEL * 4, dropout=0.2,
                                           batch_first=True, activation="gelu")
        self.enc = nn.TransformerEncoder(layer, LAYERS)
        self.fc = nn.Sequential(nn.LayerNorm(DMODEL), nn.Linear(DMODEL, 64),
                                nn.GELU(), nn.Dropout(0.3), nn.Linear(64, 1))

    def forward(self, x):
        pad_mask = (x[:, :, 0] == 0)                       # True=패딩
        z = torch.cat([self.e_brd(x[:, :, 0]), self.e_part(x[:, :, 1]), self.e_amt(x[:, :, 2])], dim=2)
        z = self.proj(z) + self.pos
        h = self.enc(z, src_key_padding_mask=pad_mask)
        m = (~pad_mask).unsqueeze(-1).float()
        pooled = (h * m).sum(1) / m.sum(1).clamp(min=1.0)  # 마스크 평균 풀링
...
def _train(Xtr, ytr, Xva, yva, Xte, vocabs):
    ...
    model = TxEncoder(vocabs).to(DEVICE)
...
def main():
    ...
    save_predictions(MODEL_NAME, oof, test_sum / C.N_FOLDS, meta=dict(
        ...
        created_by="hyunbean", notes=f"Transformer encoder (SASRec-style, maxlen{MAXLEN})"))
