"""TabM (1등 피처셋) → 'first_tabm'. RealMLP가 1등피처서 먹혔으니 TabM도 (강한 NN × 강한피처).
실행(GPU): pip install pytabkit gensim catboost → python -m src.train_first_tabm
"""
import numpy as np
...
def main():
    ...
    save_predictions(MODEL_NAME, oof, ts / C.N_FOLDS, meta=dict(cv_auc=cv, seed=C.SEED, n_folds=C.N_FOLDS,
                     feature_set="1등 FE(521) TabM", created_by="hyunbean", notes="TabM on 1st-place features"))
